...
## Added 18/08/2026 — Phase 5 gaps closed and Phase 7 production review (`feature/homepage-production`)

| # | Item | Where it surfaces | Status | Notes |
|---|---|---|---|---|
| 80 | Rental estimate: honest weak-evidence and unavailable states added; the demonstration adapter refuses a range for areas outside its table (Millfield, New England, Eastfield, West Town, Netherton) rather than guessing; the result now ends with "Request a verified rental appraisal" | `/rental-appraisal` | **Assumed** | `assessRentalReport()` in `src/lib/appraisal/types.ts`; a live provider (row 51) plugs in behind the same states |
| 81 | Occupied ("Currently let") homes publish street, district and property type only — floor area, EPC dwelling type and EPC rating are shown for marketed homes only (the public EPC register could otherwise single out one house on a street) | Property pages | **Assumed** | Independent review finding; `src/lib/listings/portfolio-listings.ts` |
| 82 | Same-street homes carry their reference in the page title so every page is distinct (e.g. "Detached house on Francis Gardens, PE1 (ref RB-342)"); "Bills" row shown only when a document states it | Property pages | **Assumed** | Review findings |
| 83 | The results line "46 of 46 homes" states the current managed count on `/properties` — CLAUDE.md says not to imply portfolio size; the count is inherent to listing the homes | `/properties` | Open | Confirm the count may show, or the list becomes available homes only |
| 84 | Live map (MapTiler) left out by owner instruction; the static schematic map ships | `/properties?view=map` | **Decided (owner)** | "leave the map" — 18/08/2026 |
| 85 | Indexing switch renamed `NEXT_PUBLIC_SITE_INDEXING` (build-time, consistent across static and dynamic routes); baseline security headers added; no Content-Security-Policy yet (needs a nonce strategy at launch) | Site-wide | **Assumed** | Review findings; CSP decision open |
| 86 | Lighthouse (launch-mode build, 18/08/2026): every page A11y 100 · Best practices 100 · SEO 100 (draft articles 69 by design: noindex) · desktop performance 99–100 · mobile performance 75–91 before the FAQ/poster changes — see `docs/evidence/phase-7/lighthouse/summary.md` for the final table | Build gate | **Assumed** | Mobile perf target 80+; the homepage hero poster was re-cut (92 KB); the FAQ moved to native `<details>` to cut hydration |
| 87 | The 3D chapter's scroll length (≈ 5 × viewport heights) exceeds the storyboard's 240–320 vh bound because the owner asked for a slower story | Homepage §4 | **Decided (owner)** | "too fast, slow it down" — 18/08/2026 |
